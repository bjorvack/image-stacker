...
class StylesheetExporterTest extends BaseTest
{
    ...
    public function testCreateFromStacker()
    {
        ...
        $packedCSS = $this->storagePath.'/'.$stacker->getName().'.css';

        $this->assertTrue(file_exists($packedImage));
        $this->assertTrue(file_exists($packedCSS));

        $css = '.unittest{display:block;background-image:url("' . $packedImage . '");}.unittest-bjorvack{width:180px;height:180px;background-position:0px0px;}';
